...
${report.context?.gyeokgukCandidates?.length ? `    <li><span class="lbl">격국 후보</span> ${esc(report.context.gyeokgukCandidates.join(' · '))} — 대표 격국 외 성립한 대안입니다</li>` : ''}
${report.context?.practitionerChoice ? `    <li><span class="lbl">역술인 선택</span> ${esc(report.context.practitionerChoice)}</li>` : ''}
...
${report.context?.yongsinSchoolSpread ? `    <li><span class="lbl">학파별 용신</span> ${esc(report.context.yongsinSchoolSpread)}${report.context.yongsinConsensus ? ` (${esc(report.context.yongsinConsensus)})` : ''}\n      <ul class="sub">\n${(report.context.yongsinSchoolDetails ?? []).map((d) => `        <li>${esc(d.school)}학파: 용신 ${esc(d.yongsin)} / 기신 ${esc(d.gisin)}${d.agree ? '' : ' (최종 용신과 불일치)'} — ${esc(d.reasoning)}</li>`).join('\n')}\n      </ul></li>` : ''}
${report.context?.strengthScoreLabel ? `    <li><span class="lbl">강약 병기</span> 점유율 ${esc(report.meter.dayMaster.verdictLabel)} ${report.meter.dayMaster.score}% / 점수제 ${esc(report.context.strengthScoreLabel)}</li>` : ''}
